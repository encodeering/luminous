...
internal class QuoteRepositoryMemoryTest {
    ...
    fun `add should add a quote to a space for a known isin` () {
        val         quotes = QuoteRepositoryMemory ()
                    ...
                    quotes.remember (quoteT1.isin)
                    quotes.add      (quoteT1)
                    quotes.add      (quoteT1) // TODO time ordering/equality currently not yet implemented
                    quotes.add      (quoteT2)
        assertThat (quotes.all      (quoteT1.isin)).containsExactly (quoteT1, quoteT1, quoteT2)
    }
